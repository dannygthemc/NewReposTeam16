...
=======
﻿package cs2212b.team16;
>>>>>>> 8b1a9e42698333111c0dbcd3cc6e8eb90d8c324d
/*
...
public class MainWindow extends JFrame {
	...
	private JComboBox<String> locBar = new JComboBox<String>(); // used to list
																// searched
																// locations
	...
	private JPanel marsPanel = new JPanel();
	private ActionListener Jcombo = new ActionListener() { // updates
															// weatherView when
															// a new item is
															// searched
		@Override
		public void actionPerformed(ActionEvent event) {
			if (((JComboBox<String>) event.getSource()).getSelectedItem() != null) {
				String item = ((JComboBox<String>) event.getSource())
...
<<<<<<< HEAD
	...
	private void initUI() {
		...
		this.setDefaultCloseOperation(EXIT_ON_CLOSE); // initiates exit on close
														// command
		...
		createFormTwo();
		createFormThree();
		tabbedPane.addTab("Current", null, currentPanel); // fills a tab window
															// with current data
		tabbedPane.addTab("Short Term", null, shortPanel); // fills a tab window
															// with short term
															// data
		tabbedPane.addTab("Long Term", null, longPanel); // fills a tab window
															// with long term
															// data
		tabbedPane.addTab("Mars", null, marsPanel); // fills a tab window with mars weather data
		// tabbedPane.addTab("Long Term", null,
		// this.createMultiForm(app.getLongTerm()),"");
		// this.setLayout(new BorderLayout());

		GroupLayout layout = new GroupLayout(this.getContentPane());
		layout.setAutoCreateGaps(true);
		layout.setAutoCreateContainerGaps(true);
		layout.setHorizontalGroup(layout.createSequentialGroup() // sets the
																	// vertical
																	// groups
...
>>>>>>> 8b1a9e42698333111c0dbcd3cc6e8eb90d8c324d
		...
		layout.setVerticalGroup(layout
				...
				// sets the vertical groups

				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE).addComponent(
								tabbedPane)).addComponent(refreshLabel)

		);

		// this.add(lbl1);
		// this.add(tabbedPane, BorderLayout.CENTER);//adds the tabbed pane to
		// the main window
...
>>>>>>> 8b1a9e42698333111c0dbcd3cc6e8eb90d8c324d
	...
	private void updateWeatherView(String cityName) {
		if (cityName.equals("--Remove?--")) {
			...
			String response = (String) JOptionPane.showInputDialog(frame,
					"Pick a location to remove:", "Remove Location",
					JOptionPane.QUESTION_MESSAGE, null,
					Arrays.copyOfRange(possibilities, 0, count), "Titan");
			if (response != null)
	...
	private void searchBoxUsed(String txt) {
		...
		JFrame frame = new JFrame();
		int result = JOptionPane.showConfirmDialog(frame, "Did you mean: "
				+ txt + " Lat:1235 Lon:1234?");
		if (JOptionPane.YES_OPTION == result) {
			int changeCurrent = JOptionPane.showConfirmDialog(frame,
					"Set your current location to " + txt + "?");
...
<<<<<<< HEAD
	...
	private JMenuBar createMenubar() {
		...
		mniFileExit.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent event) { // when click
				try {
					// write to savedList.txt
					// if it does not exist, create a new file
					FileWriter writer = new FileWriter("savedList.txt",true);
					
					// write the current location to the text file
					writer.write(app.getCurrentLocation().getName());
					for (int i=0; i<app.getMyLocations().length;i++){
						// write the saved locations to the txt file
						writer.write((app.getMyLocations())[i].getName());
						writer.write("\n");
					}
...
>>>>>>> 8b1a9e42698333111c0dbcd3cc6e8eb90d8c324d
	...
	/*
	private void createFormFour(){
		
		//section to grab data
		
		//labels used for formating panels
		JLabel lbl41 = new JLabel("Atmosphere Opacity: ");
		JLabel lbl42 = new JLabel("Temp: ");
		JLabel lbl43 = new JLabel("Pressure: ");
		JLabel lbl44 = new JLabel("Humidity: ");
		JLabel lbl45 = new JLabel("Speed: ");
		JLabel lbl46 = new JLabel("Degree: ");
	...
	*/
	...
	private void createFormThree() {
		...
		tmp = app.getLongTerm(); // grabs weatherData objects now filled with
									// data
		...
		JLabel lblcity = new JLabel(tmp[0].getName() + ", " + tmp[0].getCount()
				+ ", " + tmp[0].getLon() + ", " + tmp[0].getLat()); // displays
																	// location
																	// info
		...
		for (int i = 0; i < 7; i++) {
			...
			hold2 = new JLabel("" + tmp[i + 1].getCondit()); // holds current
																// condition
			descrip[i] = hold2;
			...
			max[i] = hold5;
			hold6 = new JLabel("" + tmp[i + 1].getSunrise());// timestamp stored
																// in Sunrise,
																// since this
																// variable was
																// not in use
...
<<<<<<< HEAD
		...
		layout.setHorizontalGroup(layout
				.createSequentialGroup()
				// sets the horizontal groups

				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.LEADING)
								// shows city data
								.addComponent(lblcity).addGap(10)
								.addComponent(lbl1).addComponent(lbl2)
								.addComponent(lbl2A).addComponent(lbl2B)
=======
		/*
		 * this method is used to fill a panel with the Long term weather Data
		 * @param none
...
>>>>>>> 8b1a9e42698333111c0dbcd3cc6e8eb90d8c324d
				...
				// the rest of the horizontal groups contain weather Data in 3
				// hour increments
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.LEADING)

						.addComponent(lblPic[0]).addComponent(date[0])
								.addComponent(temp[0]).addComponent(min[0])
								.addComponent(max[0]).addComponent(descrip[0])
								.addGap(30).addComponent(lblPic[5])
								.addComponent(date[5]).addComponent(temp[5])
								.addComponent(min[5]).addComponent(max[5])
				...
				.addGroup(
						...
						.addComponent(lblPic[1]).addComponent(date[1])
								.addComponent(temp[1]).addComponent(min[1])
								.addComponent(max[1]).addComponent(descrip[1])
								.addGap(30).addComponent(lblPic[6])
								.addComponent(date[6]).addComponent(temp[6])
								.addComponent(min[6]).addComponent(max[6])
				...
				.addGroup(
						...
						.addComponent(lblPic[2]).addComponent(date[2])
								.addComponent(temp[2]).addComponent(min[2])
								.addComponent(max[2]).addComponent(descrip[2])
								.addGap(30))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.LEADING)

						.addComponent(lblPic[3]).addComponent(date[3])
								.addComponent(temp[3]).addComponent(min[3])
								.addComponent(max[3]).addComponent(descrip[3])
								.addGap(30))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.LEADING)

						.addComponent(lblPic[4]).addComponent(date[4])
								.addComponent(temp[4]).addComponent(min[4])
								.addComponent(max[4]).addComponent(descrip[4])
		...
		layout.setVerticalGroup(layout
				...
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)// city is in its
																// own vertical
																// group
								.addComponent(lblcity))
				.addGroup(
				...
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// dates alligned vertically
								.addComponent(lbl1).addComponent(date[0])
								.addComponent(date[1]).addComponent(date[2])
								.addComponent(date[3]).addComponent(date[4]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// temps aligned vertically
								.addComponent(lbl2).addComponent(temp[0])
								.addComponent(temp[1]).addComponent(temp[2])
								.addComponent(temp[3]).addComponent(temp[4]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// min temps aligned vertically
								.addComponent(lbl2A).addComponent(min[0])
								.addComponent(min[1]).addComponent(min[2])
								.addComponent(min[3]).addComponent(min[4]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// max temps aligned vertically
								.addComponent(lbl2B).addComponent(max[0])
								.addComponent(max[1]).addComponent(max[2])
								.addComponent(max[3]).addComponent(max[4]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// descrips alligned vertically
								.addComponent(lbl3).addComponent(descrip[0])
				...
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// this is the start of the second row of
								// weather Data, pics aligned
								.addComponent(lblPic[5])
								.addComponent(lblPic[6]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)// dates aligned
								.addComponent(date[5]).addComponent(date[6]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)// temps aligned
								.addComponent(temp[5]).addComponent(temp[6]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)// min temps
																// aligned
								.addComponent(min[5]).addComponent(min[6]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)// max temps
																// aligned
								.addComponent(max[5]).addComponent(max[6]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// descrips aligned
	...
	private void createFormTwo() {
		...
		tmp = app.getShortTerm(); // grabs weatherData objects now filled with
									// data
		...
		for (int i = 0; i < 9; i++) {
			...
			lblPic[i] = hold3;
			hold4 = new JLabel(tmp[i + 1].getSunrise());// timestamp stored in
														// Sunrise, since this
														// variable was not in
														// use for ShorTerm
			time[i] = hold4;
...
>>>>>>> 8b1a9e42698333111c0dbcd3cc6e8eb90d8c324d
				...
				.addGroup(
						...
						.addComponent(lblPic[0]).addComponent(time[0])
								.addComponent(temp[0]).addComponent(descrip[0])
								.addGap(30).addComponent(lblPic[5])
								.addComponent(time[5]).addComponent(temp[5])
								.addComponent(descrip[5])
				...
				.addGroup(
						...
						.addComponent(lblPic[1]).addComponent(time[1])
								.addComponent(temp[1]).addComponent(descrip[1])
								.addGap(30).addComponent(lblPic[6])
								.addComponent(time[6]).addComponent(temp[6])
								.addComponent(descrip[6])
				...
				.addGroup(
						...
						.addComponent(lblPic[2]).addComponent(time[2])
								.addComponent(temp[2]).addComponent(descrip[2])
								.addGap(30).addComponent(lblPic[7])
								.addComponent(time[7]).addComponent(temp[7])
								.addComponent(descrip[7]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.LEADING)

						.addComponent(lblPic[3]).addComponent(time[3])
								.addComponent(temp[3]).addComponent(descrip[3])
								.addGap(30).addComponent(lblPic[8])
								.addComponent(time[8]).addComponent(temp[8])
				...
				.addGroup(
						...
						.addComponent(lblPic[4]).addComponent(time[4])
								.addComponent(temp[4]).addComponent(descrip[4])
		...
		layout.setVerticalGroup(layout
				...
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// times aligned vertically
								.addComponent(lbl1).addComponent(time[0])
								.addComponent(time[1]).addComponent(time[2])
								.addComponent(time[3]).addComponent(time[4]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// temps aligned vertically
				...
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// times aligned
								.addComponent(time[5]).addComponent(time[6])
								.addComponent(time[7]).addComponent(time[8]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// temps aligned
								.addComponent(temp[5]).addComponent(temp[6])
								.addComponent(temp[7]).addComponent(temp[8]))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// descrips aligned
...
<<<<<<< HEAD
	...
	private void createForm() {
		...
		JLabel lblcity = new JLabel(tmp.getName() + ", " + tmp.getCount()
				+ ", " + tmp.getLon() + ", " + tmp.getLat()); // displays
																// location info
		...
		layout.setHorizontalGroup(layout
				.createSequentialGroup()
				// sets horizontal groups
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.LEADING)
								// holds all the identifier labels
								.addComponent(lblcity).addComponent(lblPic)
								.addComponent(lbldescrip).addComponent(lbltemp)
								.addComponent(lblmin).addComponent(lblmax)
								.addComponent(lblspeed).addComponent(lbldir)
								.addComponent(lblpress).addComponent(lblhumid)
								.addComponent(lblrise).addComponent(lblset)

				)
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.LEADING)
								// sets wetaher Data labels

								.addComponent(lbldescrip2)
								.addComponent(lbltemp2).addComponent(lblmin2)
								.addComponent(lblmax2).addComponent(lblspeed2)
								.addComponent(lbldir2).addComponent(lblpress2)
								.addComponent(lblhumid2).addComponent(lblrise2)
								.addComponent(lblset2)));
		layout.setVerticalGroup(layout
				.createSequentialGroup()
				// sets verticsal groups
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE) // city is on
																// its own
								.addComponent(lblcity))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE) // pic is on its
																// own
								.addComponent(lblPic))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								// the rest have the identifier, followed by the
								// actual data
				...
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(lbltemp).addComponent(lbltemp2))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(lblmin).addComponent(lblmin2))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(lblmax).addComponent(lblmax2))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(lblspeed).addComponent(lblspeed2))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(lbldir).addComponent(lbldir2))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(lblpress).addComponent(lblpress2))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(lblhumid).addComponent(lblhumid2))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(lblrise).addComponent(lblrise2))
				.addGroup(
						layout.createParallelGroup(
								GroupLayout.Alignment.BASELINE)
								.addComponent(lblset).addComponent(lblset2)));
